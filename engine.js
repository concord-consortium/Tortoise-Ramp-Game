...
Prims = {
  ...
  substring: function(s, p1, p2) {
    if (p1 < 0) {
      throw new NetLogoException("Runtime error: " + p1 + " is less than zero.");
    }
    if (p2 < p1) {
      throw new NetLogoException("Runtime error: " + p2 + " is less than " + p1 + ".");
    }
    if (p2 > s.length) {
      throw new NetLogoException("Runtime error: " + p2 + " is too big for " + s + ", which is only of length " + s.length + ".");
    }
    return s.substring(p1, p2);
  },
  replaceitem: function(index, target, val) {
    var retn;
    if (typeof target === "string") {
      return target.slice(0, index) + val + target.slice(index + 1);
    } else if (typeIsArray(target)) {
      retn = target.slice();
      retn.splice(index, 1, val);
      return retn;
    } else {
      throw new NetLogoException(typeof target);
    }
  },
